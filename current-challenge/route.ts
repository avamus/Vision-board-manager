...
export async function GET(request: Request) {
  ...
  console.log("Challenge API GET request received for memberId:", memberId);
  ...
  try {
    ...
    const { rows } = await client.query(
      'SELECT * FROM current_challenge WHERE memberstack_id = $1 ORDER BY created_at DESC LIMIT 1',
      [memberId]
    );
    
    console.log(`Found ${rows.length} challenge records for memberId:`, memberId);
    ...
    return NextResponse.json({ error: 'Failed to load challenge data', details: error?.toString() }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.log("Challenge API POST request received with URL memberId:", urlMemberId);
    ...
    const { rows: existingRows } = await client.query(
      'SELECT id FROM current_challenge WHERE memberstack_id = $1',
      [finalMemberId]
    );
    ...
    return NextResponse.json({ error: 'Failed to save challenge data', details: err?.toString() }, { status: 500 });
  }
}
  ...
  try {
    ...
    const query = `
      ...
      SET 
        ...
        icon_id = $7,
        updated_at = NOW()
      WHERE id = $8 AND memberstack_id = $9
      RETURNING *
    `;
    ...
    const error = err as Error;
    return NextResponse.json({ error: 'Failed to update challenge data', details: error?.toString() }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const { rowCount } = await client.query(
      'DELETE FROM current_challenge WHERE id = $1 AND memberstack_id = $2',
      [id, memberstack_id]
    );
